...
- `Draft Experience`: Enables previews when no current or legacy preference is configured, preserving explicit opt-outs without config migration. A two-second leading/trailing throttle sends the first eligible frame immediately and then only the latest accumulated text, producing an operator-accepted rhythm on short and long answers while reducing Telegram rate-limit pressure.
